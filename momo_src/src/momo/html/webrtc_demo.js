...
function cmd_Turn_LED_on() {
  let textData = "$CMDld1xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_Turn_LED_off() {
  let textData = "$CMDld0xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_Turn_LED_with_time() {
  let time = time1.value;
  let textData = "$CMDldt" + time + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_Turn_Mist_on() {
  let textData = "$CMDmi1xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_Turn_Mist_off() {
  let textData = "$CMDmi0xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_Turn_Mist_with_time() {
  let time = time2.value;
  let textData = "$CMDmit" + time + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_date() {
  let val = 1;
  let textData = "$CMDct10" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_month() {
  let val = 1;
  let textData = "$CMDct20" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_year() {
  let val = 23;
  let textData = "$CMDct30" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_hour() {
  let val = 1;
  let textData = "$CMDct40" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_minute() {
  let val = 1;
  let textData = "$CMDct50" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_current_time_mem() {
  let textData = "$CMDct9xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
////////////////////////////////////////////////////////////////////////
function cmd_alarm1_time_date() {
  let val = 1;
  let textData = "$CMDa110" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm1_time_month() {
  let val = 1;
  let textData = "$CMDa120" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm1_time_year() {
  let val = 23;
  let textData = "$CMDa130" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm1_time_hour() {
  let val = 1;
  let textData = "$CMDa140" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm1_time_minute() {
  let val = 1;
  let textData = "$CMDa150" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm1_time_mem() {
  let textData = "$CMDa19xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
//////////////////////////////////////////////////////////////////////////////
function cmd_alarm2_time_date() {
  let val = 1;
  let textData = "$CMDa210" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm2_time_month() {
  let val = 1;
  let textData = "$CMDa220" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm2_time_year() {
  let val = 23;
  let textData = "$CMDa230" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm2_time_hour() {
  let val = 1;
  let textData = "$CMDa240" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm2_time_minute() {
  let val = 1;
  let textData = "$CMDa250" + val.toString() + "\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
...
function cmd_alarm2_time_mem() {
  let textData = "$CMDa29xxx\r\n";
  if (dataChannel == null || dataChannel.readyState != "open") {
    return;
  }
  dataChannel.send(new TextEncoder().encode(textData));
